...
parser.add_argument("--model", type=str, default="rwkv_unet", help="model_name")
parser.add_argument("--max_iterations", type=int, default=30000, help="maximum epoch number to train")
parser.add_argument("--batch_size", type=int, default=6, help="batch_size per gpu")
parser.add_argument("--deterministic", type=int, default=1, help="whether use deterministic training")
parser.add_argument("--base_lr", type=float, default=0.015, help="segmentation network learning rate")
...
def train(args, snapshot_path):
    ...
    db_train = BaseDataSets(
        ...
        split="train",
        num=None,
        transform=transforms.Compose([SequenceAugment_Ours(args.patch_size, args, split='train', N=5)]),
        # transform=transforms.Compose([WeakStrongAugment_Ours(args.patch_size)]),
    )
    print("ACDC train total slices:", len(db_train))

    db_val = BaseDataSets(base_dir=args.root_path, split="val")
    db_test = BaseDataSets(base_dir=args.root_path, split="test")
    print("ACDC val total slices:", len(db_val))
    print("ACDC test total slices:", len(db_test))
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for i in range(N - 1):
                ...
                ref_comp_loss, ref_as_weight = get_comp_loss(weak=ref_outputs_soft, strong=ref_outputs_strong_soft)
                unsup_loss_cedice = (
...
if __name__ == "__main__":
    ...
    snapshot_path = "./logs_ACDC_1%_N5_base_lr_{}_rwkv_unet/{}_{}_labeled/{}".format(
        args.base_lr, args.exp, args.labeled_num, args.model)
